<template>
    ...
    <MainBody>
        <el-row :gutter="15">
            <el-col :class="['animate__animated',menuPageConfig.isMenuHoverIndex==index?'animate__bounceIn':'']" 
                v-for="(item,index) in menuList" :xl="4" :md="6" :sm="8" :xs="12" :key="index" 
                @mouseover="menuPageConfig.isMenuHoverIndex = index"
                >
                    ...
                    <template #header>
                        <div class="card-header">
                            ...
                            <div class="right-type">
                                <span v-if="item.type==0">(目录)</span>
                                <span v-if="item.type==1">(菜单)</span>
                                <span v-if="item.type==2">(按钮)</span>
                                <span style="color:cornflowerblue;margin-left: 20px;" v-if="item.status==0">禁用</span>
                                <span style="color:cornflowerblue;margin-left: 20px;" v-if="item.status==1">启用</span>
                            </div>
                        </div>
                    </template>
                    <div class="handle-area">
                        <el-button size="small" type="info" plain round @click="detailMenu(item,index,1)">详情</el-button>
                        <el-button size="small" type="warning" plain round @click="editMenu(item,index,1)">编辑</el-button>
                        <el-popconfirm 
                        ...
                        <!-- <el-button size="small" type="primary" plain round @click="showSubList(item,index)">子项</el-button> -->
                    </div>
                </el-card>
            </el-col>
            <el-col :xl="4" :md="6" :sm="8" :xs="12">
                <el-card shadow="hover" style="margin-top:8px;margin-bottom:8px" @click="createMenu(1)">
                    <template #header>
                        <div class="card-header" style="justify-content:center">
                            <span style="font-weight: bold;color: #444;margin-right: 10px;">新建</span>
                            <el-icon class="menu-icon" style="color: blueviolet;"><circle-plus /></el-icon>
                        </div>
                    </template>
                    <div class="handle-area">
                        <el-button size="small" type="primary">目录</el-button>
    ...
    <div :class="['menu-children-mask animate__animated',showChildrenMenuMask==1?'animate__fadeInBottomLeft':showChildrenMenuMask==2?'animate__fadeOutBottomRight':'']">
        <el-table
            :data="subMenuList"
            row-key="id"
            border
            ...
            >
            <el-table-column prop="name" align="center" label="菜单名称" width="180" >
                <template #default="scope">
                    <template v-if="scope.row.type==2">
                        <el-button size="small" type="warning">{{scope.row.name}}</el-button>
                    </template>
        ...
        <div class="bottom-close-btn">
            <el-button @click="showChildrenMenuMask=2" type="primary" plain>关闭</el-button>
        </div>
    </div>
            ...
            <div>
                <el-collapse v-model="activeName" accordion>
                    <el-collapse-item v-for="(item,index) in subMenuList" :name="index" :key="index">
                        <template #title>
                            <el-icon class="col-icon edit" @click.stop="editMenu(item,index,2)"><edit /></el-icon>
                            <el-popconfirm 
                                confirm-button-text="OK"
                                cancel-button-text="No"
                                icon-color="red"
    ...
    >
        ...
        >
            ...
            <el-form-item class="menu-form-item" label="权限类型" prop="type">
                <el-select v-model="formProp.type" placeholder="请设置权限类型">
                    <el-option
                        v-for="item in rightOption"
                        :key="item.value"
                        :label="item.label"
...
<script setup>
    ...
    const menuPageConfig = reactive({
        isMenuHoverIndex:-1,//控制主目录的动画效果
        subMenuList:[],//子菜单数据存储变量
    })
    ...
    const showChildrenMenuMask = ref(false)
    ...
    const controFormObj = reactive({
        ...
        menuDialogVisible:false,//控制菜单及目录表单的显示
        showBtnDialog:false,//控制按钮表单的显示
    })
    const menuFormRef = ref()
    const btnFormRef = ref()

    //查看一级目录详情
    const detailMenu = (item,index)=>{
        
        showChildrenMenuMask.value = 1
